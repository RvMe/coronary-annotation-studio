"""Native LPS geometry and frame-based CPR viewing.

Arrays are (N,H,W), with columns along normal and rows along binormal.
Frames contain native physical LPS coordinates. A CPR image affine describes
only its local grid. Frame views sample native HU without changing source data;
dense nonlinear mappings are implemented separately in coordinate_field.py.
"""
...
def _read_image(path: str | Path) -> sitk.Image:
    ...
    with tempfile.TemporaryDirectory(prefix="CAS-decode-") as directory:
...
def load_case(case_manifest_path, *, verify_checksums=True):
    from .package import load_neutral_case
    return load_neutral_case(case_manifest_path, verify_checksums=verify_checksums)
