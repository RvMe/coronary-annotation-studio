...
EPS = 1e-6
SCHEMA_VERSION = "cas-annotations-1.0"
...
def _normalise_annotation(annotation: dict) -> dict:
    ...
    if not valid_path(result.get("path_id")):
...
def validate_state(state: dict) -> None:
    ...
    for marker in state["markers"]:
        ...
        if not valid_path(marker.get("path_id")) or _number(marker.get("s_mm"), "s_mm") < 0:
            raise ValueError("Invalid marker")
    if len(marker_ids) != len(set(marker_ids)):
        raise ValueError("Duplicate marker ID")
    for interval in state["rereview_intervals"]:
        if not valid_path(interval.get("path_id")):
...
def add_marker(state, path_id, s_mm) -> dict:
    ...
    if not valid_path(path_id) or s_mm < 0:
...
def mark_case_complete(state, report, *, allow_incomplete=False):
    ...
    result["completion"] = {
        "schema_version": "cas-completion-1.0",
