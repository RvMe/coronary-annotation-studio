...
class AnnotationStore:
    ...
    def __init__(self, db_path):
        ...
        # Inspect before enabling WAL or running DDL. Legacy stores are never
        # migrated in place; users must import a verified complete export.
        if self.connection.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='streams'").fetchone():
            for row in self.connection.execute("SELECT state_json FROM streams"):
                if json.loads(row[0]).get("schema_version") != "cas-annotations-1.0":
                    self.connection.close()
                    raise ValueError("Legacy/unknown database: import a complete export into a new project; do not open the old database")
        ...
        self.connection.executescript("""
            CREATE TABLE IF NOT EXISTS project_identity (singleton INTEGER PRIMARY KEY CHECK(singleton=1), project_id TEXT NOT NULL);
        ...
        """)
        self._sources = {}
        sidecar = self.db_path.parent / "project.json"
        if sidecar.exists():
            try:
                self._check_project(json.loads(sidecar.read_text(encoding="utf-8")))
            except BaseException:
                self.connection.close()
                raise

    def _check_project(self, source):
        pid = source.get("project_id")
        row = self.connection.execute("SELECT project_id FROM project_identity WHERE singleton=1").fetchone()
        if row and pid != row[0]:
            raise ValueError("Project identity mismatch: use the project's separate database")
        if pid is not None:
            from .projects import validate_id
            validate_id(pid)
            if row is None:
                # Prevent binding a pre-existing source-less stream to a new project.
                for stream in self.connection.execute("SELECT state_json FROM streams"):
                    if json.loads(stream[0])["source"].get("project_id") != pid:
                        raise ValueError("Existing records cannot be rebound to another project")
                self.connection.execute("INSERT INTO project_identity VALUES(1,?)", (pid,))
    ...
    def _write(self, state, action, details, undo, redo, revision):
        self._check_project(state["source"])
